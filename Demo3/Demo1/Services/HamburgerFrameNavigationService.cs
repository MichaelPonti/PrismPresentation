...
{
	/// <summary>
	/// This class is a copy of the FrameNavigationService. It has been updated to use the viewmodel type name
	/// as the session state key instead of using a stack of views. Decided to make a copy of the FrameNavigationService
	/// instead of modifying anything the libraries themselves so that can just bring in the nuget packages.
	/// Override OnCreateNavigationService in the PrismUnityApplication instance to create this object. Keep using
	/// the GetPageType that is declared in PrismUnityApplication can still be used.
	/// </summary>
	public class HamburgerFrameNavigationService : INavigationService
	{
		...
		public HamburgerFrameNavigationService(IFrameFacade frame, Func<string, Type> navigationResolver, ISessionStateService sessionStateService)
		{
			_frame = frame;
		...
		public bool Navigate(string pageToken, object parameter)
		{
			...
			if (pageType == null)
			{
				var resourceLoader = ResourceLoader.GetForCurrentView(InfrastructureResourceMapId);
		...
		private void NavigateToCurrentViewModel(NavigatedToEventArgs e)
		{
			...
			var viewModelKey = e.SourcePageType.Name;
		...
		private void NavigateFromCurrentViewModel(NavigatingFromEventArgs e, bool suspending)
		{
			...
			var viewModelKey = departingView.GetType().Name;
			var frameState = _sessionStateService.GetSessionStateForFrame(_frame);
